...
static int print_stats(
    ...
    for (i=0; i<data->nb_queues; i++) {
        printf("Writing core %d: %s ",
                data->write_core_stats[i].core_id,
                data->write_core_stats[i].output_file);
        printf("(%s)\n", bytes_format(
                    data->write_core_stats[i].current_file_bytes));
    }
